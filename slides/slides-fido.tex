...
LOG: '> Decoded clientDataJSON:
{
  ...
  "type": "webauthn.create"               ← ユーザ登録のときは\texttt{webauthn.create}固定。
}'
\end{verbatim}
...
[
  {
    ...
    "authData": {                         /* 署名対象、つまりCredential Public Keyのフィールド */
      "type": "Buffer",
      "data": "SZYN5YgOjGh0NBcPZHZg..."   ← Credential Public Keyとそのメタ情報 (本当はバイナリ)
    }
  }
]'
\end{verbatim}
...
\begin{frame}
 今回触ってみたことがWebAuthnの全体像ではないことに注意。あくまで1つの例と考えてもらってもいい。
\end{frame}
